...
const Home = () => {
  ...
  return (
    <div className="home-page overflow-hidden">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-20 pb-10"><div className="flex flex-wrap items-end justify-between gap-4 mb-7"><div><p className="eyebrow">Shop by intention</p><h2 className="font-display text-4xl text-primary-900 leading-none mt-3">A considered edit<br />for everyday care.</h2></div><Link to="/products" className="inline-flex items-center gap-2 text-sm font-bold text-primary-800 hover:text-accent-700">Explore all collections <FiArrowUpRight /></Link></div><div className="grid grid-cols-2 lg:grid-cols-5 gap-3">{categories.filter((item) => item.isFeatured).map((category) => <Link key={category._id} to={`/products?category=${category._id}`} className="home-collection-card group"><img src={CATEGORY_VISUALS[category.name] || CATEGORY_VISUALS["Medicines & Wellness"]} alt="" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = CATEGORY_VISUALS["Medicines & Wellness"]; }} /><span className="home-collection-shade" /><span className="relative flex h-full flex-col justify-end p-4 text-white"><span className="text-xl">{category.icon}</span><span className="mt-2 text-sm font-extrabold">{category.name}</span><span className="mt-1 text-[10px] leading-4 text-white/75">{category.subcategories?.length || 0} curated subcategories</span></span></Link>)}</div></section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12"><div className="flex flex-wrap items-end justify-between gap-4 mb-7"><div><p className="eyebrow"><FiStar className="inline mr-1" /> The Aurevia edit</p><h2 className="font-display text-3xl text-primary-900 mt-2">Pharmacy, pared back to what matters.</h2></div><Link to="/products" className="text-sm font-bold text-primary-700 hover:text-accent-700">View pharmacy <FiArrowUpRight className="inline" /></Link></div>{loading ? <Loader /> : loadError ? <div className="rounded-3xl border border-amber-200 bg-amber-50 px-5 py-8 text-sm text-amber-900">The catalogue is taking a moment to reconnect. Visit the shop to retry.</div> : featured.length === 0 ? <p className="text-gray-500 text-sm">No products are available yet.</p> : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">{featured.map((product) => <ProductCard key={product._id} product={product} />)}</div>}</section>
    </div>
  );
};
