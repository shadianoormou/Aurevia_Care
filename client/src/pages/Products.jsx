    ...
    <div className="catalog-page pb-16">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 -mt-7 relative z-10">
        <div className="catalog-rail" aria-label="Shop by primary category">
          {featuredCategories.map((item) => <button key={item._id} onClick={() => chooseCategory(item._id)} className={`catalog-category group ${category === item._id ? "catalog-category-active" : ""}`}><img src={CATEGORY_VISUALS[item.name] || CATEGORY_VISUALS["Medicines & Wellness"]} alt="" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = CATEGORY_VISUALS["Medicines & Wellness"]; }} className="absolute inset-0 w-full h-full object-cover opacity-45 group-hover:scale-110 transition-transform duration-700" /><span className="catalog-category-overlay" /><span className="relative flex items-end justify-between gap-3 h-full"><span><span className="text-xl">{item.icon}</span><span className="block mt-4 text-sm font-extrabold">{item.name}</span><span className="block mt-1 text-[11px] text-white/75 line-clamp-2">{item.description}</span><span className="block mt-3 text-[10px] font-bold uppercase tracking-[.13em] text-accent-200">{item.subcategories?.length || 0} collections</span></span><FiChevronRight className="mb-1 opacity-70" /></span></button>)}
        </div>
      </section>
